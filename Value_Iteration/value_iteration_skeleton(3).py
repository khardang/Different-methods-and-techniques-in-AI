...
def value_iteration() -> Any:
    ...
    delta_finish = (constants.epsilon*(1-constants.gamma))/constants.gamma
    repeat = True
    while repeat:
        #maximum change of utility of any state un an iteration
        delta=0
        copy_utility = np.copy(utility)


        for state in range(0,constants.number_states):
            max_utility = float(-999999999)
            for actions in moves:
                #outcomes er en liste av statsene den gitte action kan gi fra denne staten
                outcomes = get_outcome_states(state,actions)

                temp_utility=0
                for outcomeState in outcomes:
                    #Probability er sannsynligheten for å komme til outcomeState fra state ved en gitt action
                    probability = get_transition_probability(state,actions,outcomeState)
                    #Utility_outcome er utility for neste state
                    utility_outcome = copy_utility[outcomeState]

                    temp_utility = temp_utility + probability*utility_outcome

                if temp_utility > max_utility:
                    max_utility = temp_utility

            utility[state] = get_reward(state) + constants.gamma * max_utility
            if abs(utility[state]- copy_utility[state])> delta:
                delta = abs(utility[state]- copy_utility[state])
        if delta < delta_finish:
            repeat = False
    ...
    for state in range(0, constants.number_states):
        ...
        for actions in moves:
            ...
            for outcomeState in outcomes:
                ...
                probability = get_transition_probability(state, actions, outcomeState)
                # Utility_outcome er utility for neste state
                utility_outcome = value_table[outcomeState]

                temp_utility = temp_utility + probability * utility_outcome
            if temp_utility > max_utility:
                max_utility = temp_utility
                best_move = actions
        all_moves[state]={best_move}
        print(all_moves)
...
def main() -> None:
    ...
    print("  %2.2f | %2.2f   | %2.2f | %2.2f " %(value_table[0],value_table[1],value_table[2],value_table[3]))
    print("-----------------------------")
    print("  %2.2f | %2.2f | %2.2f | %2.2f " % (value_table[4], value_table[5], value_table[6], value_table[7]))
    print("-----------------------------")
    print("  %2.2f | %2.2f   | %2.2f | %2.2f " % (value_table[8], value_table[9], value_table[10], value_table[11]))
    print("-----------------------------")
    print("%2.2f | %2.2f | %2.2f | %2.2f " % (value_table[12], value_table[12], value_table[14], value_table[15]))
